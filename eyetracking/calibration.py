...
def wait_for_face_and_countdown(cap: cv2.VideoCapture, gaze_estimator: GazeEstimator, sw: int, sh: int, dur: int = 2):
    """
    Waits for a face to be detected (not blinking), then does a countdown ellipse.
    
    Args:
        cap (cv2.VideoCapture): VideoCapture object
        gaze_estimator (GazeEstimator) : GazeEstimator object
        sw (int): screen width
        sh (int): screen height
        dur (int): wait duration
...
def run_9_point_calibration(gaze_estimator, camera_index=0):
    ...
    Args:
        gaze_estimator (GazeEstimator): Uncalibrated gaze estimator
        camera_index (int): Index of camera being used (default 0)
    """
    root = tk.Tk()
...
def fine_tune_kalman_filter(gaze_estimator, kalman: cv2.KalmanFilter, camera_index=0):
    """
    ...
    Args:
        gaze_estimator (GazeEstimator): Calibrated gaze estimator
